...
categorical_pipe = Pipeline([("numeric_mi", numeric_missing_indicator_), ("categorical_mi", missing_imputer), ("rare_label_enc", rare_label_encoder)])
...
processed_train_df = pd.get_dummies(processed_train_df, columns=config.CATEGORICAL_VARS, drop_first=True)
processed_test_df = pd.get_dummies(processed_test_df, columns=config.CATEGORICAL_VARS, drop_first=True)

missing_onehot_cols = list(set(processed_train_df.columns) - set(processed_test_df.columns))
for colname in missing_onehot_cols:  processed_test_df[colname] = 0   
...
numeric_pipeline = Pipeline([("median_imputer", numerical_median_imputer), ("min_max_scaler", min_max_scaler)])
numeric_col_transformer = ColumnTransformer([("numeric_transforms", numeric_pipeline, config.NUMERICAL_VARS)])

processed_train_df[config.NUMERICAL_VARS] = numeric_col_transformer.fit_transform(processed_train_df)
processed_test_df[config.NUMERICAL_VARS] = numeric_col_transformer.fit_transform(processed_test_df)
